document.addEventListener('DOMContentLoaded', function() {
    ...
    const apiUrl = `https://temirovv.uz/api/product-detail/${productId}`;  // Adjust the URL to fetch the specific product

    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
            // Update the page with the product details
            document.getElementById('product-image').src = 'https://temirovv.uz/' +  data.image[0].image;
